...
if __name__ == '__main__':
    testApp = wx.App()
    testFrame = wx.Frame(None, -1, "Dummy wx frame for testing printout.py")

    from wx import grid
    testgrid = grid.Grid(testFrame)
    testgrid.CreateGrid(5, 5)
    for i in range(5):
        d = i + 1
        testgrid.SetColLabelValue(i, 'Spalte %d' % d)
        testgrid.SetRowLabelValue(i, 'Zeile %d' % d)
        for j in range(5):
            e = j + 1
            testgrid.SetCellValue(i, j, 'Zelle %02d.%02d' % (d, e))
    testgrid.SetCellBackgroundColour(0, 0, wx.RED)
    testgrid.SetCellBackgroundColour(4, 4, wx.GREEN)
    testgrid.SetCellBackgroundColour(3, 4, wx.BLUE)
    testgrid.SetCellBackgroundColour(4, 3, wx.YELLOW)
    testgrid.SetColSize(0, 71)
    testgrid.SetColSize(1, 71)
    testgrid.SetColSize(2, 107)
    testgrid.SetColSize(3, 143)
    testgrid.SetColSize(4, 178)
    ...
    grdprt = PrintGrid(testFrame, testgrid, rowLabels=False, colLabels=False)
    grdprt.SetAttributes()

    table = grdprt.GetTable()
    table.SetLandscape()

    grdprt.Preview()
    testFrame.grid = testgrid
    testFrame.Show(True)
    testApp.MainLoop()
